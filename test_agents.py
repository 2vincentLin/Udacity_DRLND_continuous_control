...
class Agent():
    def __init__(self, state_size, action_size, num_agents, num_process, random_seed= 0.0):
        ...
        self.agents = [base_agent(state_size, action_size, random_seed) for _ in range(num_agents)]
        ...
        if len(self.memory) > BATCH_SIZE:
            with ThreadPoolExecutor(max_workers=self.num_process) as executor:
                for i in range(self.num_agents):
                    ...
                    executor.submit(self.agents[i].learn, experiences, GAMMA)
    ...
    def load_weights(self):
        if torch.cuda.is_available():
            for i in range(self.num_agents):
                self.agents[i].actor_local.load_state_dict(torch.load('mode_weights/agent{}_actor_local.pth'.format(i)))
                self.agents[i].critic_local.load_state_dict(torch.load('mode_weights/agent{}_critic_local.pth'.format(i)))
                self.agents[i].actor_target.load_state_dict(torch.load('mode_weights/agent{}_actor_target.pth'.format(i)))
                self.agents[i].critic_target.load_state_dict(torch.load('mode_weights/agent{}_critic_target.pth'.format(i)))
        else:
            for i in range(self.num_agents):
